...
/** I take a string and I return that string as an array, split on each character **/
function parse(s) {
    return s.split("");
}


/** takes a character, returns true if the character is a vowel, false if it is not **/
...
/** takes a string, returns true if the string container three vowels, and false if it doesn't **/
...
/** I take a string and return true if there are any double letters in the string **/
function containsDoubleLetters(s){
...
/** I take a string and return whether that string contains the pairs ab, cd, pq, or xy **/
function containsDipthongs(s) {
...
function containsPairNoOverlap(s) {

    var input = parse(s);

    //loop through the string and get a pair out of the string
    for(var i = 0, l = input.length - 3; i < l;i++) {

        var pair = input[i] + input[i+1];
        var match = s.indexOf(pair,i+2);
...
/** I take a string, and return true if there are two letters the same with exactly one letter between them**/
function containsLetterSandwich(s) {

    var input = parse(s);

    for(var i = 0, l = input.length-2; i < l;i++) {
...
function checkNiceCount(input) {

    var string = readStrings(input);
    var count = 0;

    for( var i = 0; i < string.length;i++) {
        if ( containsThreeVowels(string[i]) && containsDoubleLetters(string[i]) && !containsDipthongs(string[i])) {
            count ++;
        }
    }
...
function checkNicerCount(input) {

    var string = readStrings(input);
    var count = 0;

    for( var i = 0; i < string.length;i++) {
        if ( containsPairNoOverlap(string[i]) && containsLetterSandwich(string[i])) {
            count ++;
        }
    }
...
document.write("<p>contains contains three vowels: " + containsThreeVowels(input) + "</p>");

document.write("<p>contains contains double letters: " + containsDoubleLetters(input) + "</p>");
document.write("<p>contains dipthings: " + containsDipthongs(input) + "</p>");
document.write("<p>contains pairs no overlap: " + containsPairNoOverlap("aabcdefgfg") + "</p>");
document.write("<p>contains letter sandwich: " + containsLetterSandwich("abcdefghih") + "</p>");

document.write("<p>answer: " + checkNiceCount(input) + "</p>");
document.write("<p>answer: " + checkNicerCount(input) + "</p>");
