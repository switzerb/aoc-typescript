...
function createGrid(size) {
    ...
    for(var i = 0; i < size;i++){
        ...
        for( var j = 0; j < size; j++){
            grid[i][j] = 0; //brightness is set to zero
        }
    }
}

function parseGridPoint(s){
...
function setGridPoint(x, y, value) {
    grid[x][y] = value;
}
...
function executeInstructions(xStart,yStart,xEnd,yEnd,type){

    var across = xEnd - xStart;
    var down = yEnd - yStart;
        ...
        for(var i = 0; i <= across;i++){
            for( var j = 0; j <= down; j++){
                ...
                if(type == "on") {
                    var gridvalue = getGridPoint(i + xStart,j + yStart);
                    setGridPoint(i + xStart,j + yStart, gridvalue + 1);
                    ...
                    var gridvalue = getGridPoint(i + xStart,j + yStart);
                    setGridPoint(i + xStart, j + yStart, gridvalue + 2);
                }
            }
        }
}
...
function main(s) {
    ...
    for(var i = 0;i < instructions.length;i++) {
        ...
        if(instructions[i][1] == "on"){

            var startpoint = parseGridPoint(instructions[i][2]);
            var endpoint = parseGridPoint(instructions[i][4]);

            executeInstructions(startpoint[0],startpoint[1],endpoint[0],endpoint[1],"on");
        } else if (instructions[i][1] == "off") {

            var startpoint = parseGridPoint(instructions[i][2]);
            var endpoint = parseGridPoint(instructions[i][4]);

            executeInstructions(startpoint[0],startpoint[1],endpoint[0],endpoint[1],"off");
        } else if (instructions[i][0] == "toggle") {

            var startpoint = parseGridPoint(instructions[i][1]);
            var endpoint = parseGridPoint(instructions[i][3]);
            executeInstructions(startpoint[0],startpoint[1],endpoint[0],endpoint[1],"toggle");
        }
    }
...
//document.write("<p>" + comprehendInstructions(input) + "</p>");
//document.write("<p>grid point 0,0" + getGridPoint(0,0));
//setGridPoint(0,0,true);
//document.write("<p>grid point 0,0" + getGridPoint(0,0));
//document.write(parseGridPoint("123,345"));
